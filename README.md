...
- `VTB` - Банк ВТБ (ПАО)
- `Tinkoff` - АО Тинькофф Банк
- `Raiffeisen` - АО Райффайзенбанк
- `Alfa` - АО Альфа-Банк
...
def verify_callback_signature(request_data, signature_header, secret_key_b64):
    """
    Проверяет подпись callback запроса
    """
...
def webhook():
    ...
    if not verify_callback_signature(data, signature, timestamp, SECRET_KEY_B64):
        return jsonify({'error': 'Invalid signature'}), 401
    
    # Обработка уведомления о транзакции
    if 'transaction_id' in data:
        transaction_id = data['transaction_id']
        status = data['status']
        amount = data['amount']
        
        print(f"Транзакция {transaction_id}: {status}, сумма: {amount}")
    ...
    # Обработка уведомления о диспуте
    elif 'dispute' in data:
        dispute = data['dispute']
        transaction = data['transaction']
        
        print(f"Диспут {dispute['id']}: {dispute['status']}")
        
        # Ваша логика обработки диспута
        if dispute['status'] == 'completed_towards_merchant':
            # Диспут решен в пользу мерчанта
            pass
        elif dispute['status'] == 'completed_towards_trader':
            # Диспут решен в пользу трейдера
            pass
